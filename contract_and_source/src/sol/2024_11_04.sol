...
library TickMath {
    ...
    function abs(int24 x) internal pure returns (int256) {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    event Harvest(uint256 indexed pid);  // This is event of withdraw.
    ...
    function getTokenAmounts(uint256 tokenId) public view returns (uint256 amount0, uint256 amount1) {
        // Get position info
        ( 
            , 
            , 
        ...
        // Calculate amount0 and amount1 using Uniswap's LiquidityAmounts library
    ...
    function deposit(uint256 _pid) public nonReentrant {
        ...
        (_amount0, _amount1) = getTokenAmounts(_pid);
        ...
        nftManager.transferFrom(address(msg.sender), address(this), _pid);
        ...
        totalLPToken = totalLPToken.add(amount);  
        updatePool(_pid, address(msg.sender));                          // amount of total staking NFT token will be added.
        emit Deposit(_pid);
    }

    function getPerUserInfo() public view returns (uint256, uint256, UserInfo[] memory) {
        uint256 eachStackedUserAmount;
        uint256 eachEarnedUserAmount;
        UserInfo[] memory eachUserInfo = userInfo[msg.sender];
        for(uint256 i = 0; i < eachUserInfo.length; i++) {
            eachStackedUserAmount = eachStackedUserAmount + eachUserInfo[i].amountPerNFT;
            eachEarnedUserAmount = eachEarnedUserAmount + eachUserInfo[i].rewardAmountHistory;
        }
        return (eachStackedUserAmount, eachEarnedUserAmount, eachUserInfo);
    }
    ...
    function withdraw(uint256 _pid) public nonReentrant {
        DepositInfo storage myDeposit =  depositInfo[msg.sender][_pid][0];
        require(block.timestamp.sub(myDeposit.startDeposit) >= MAX_HARVEST_INTERVAL, "You can withdraw after 1 day");
        require(block.timestamp.sub(myDeposit.startDeposit) >= DEFAULT_HARVEST_INTERVAL, "You can withdraw after 1 day in minimum");
        require(myDeposit.owner == msg.sender, "You cannot withdraw because you are owner of LP NFT");

        // When withdraw, user will receive their LP NFT token.
        nftManager.transferFrom(address(this), msg.sender, _pid);        
        ...
        IERC20(haha).transfer(address(msg.sender), myDeposit.rewardAmount);    // User will get rewrad token.
        
        totalAllocPoint = totalAllocPoint.sub(1);                               // count of total staking NFT token will be decrased.
        totalLPToken = totalLPToken.sub(myDeposit.amount);   // amount of total staking NFT token will be added.

        UserInfo[] storage userInfos = userInfo[msg.sender];
        uint256 length = userInfos.length;

        // Find the index of the UserInfo with the matching `pid`
        for (uint256 i = 0; i < length; i++) {
            if (userInfos[i].pId == _pid) {
                // Swap the last element into the place of the element to be removed
    ...
    function getAllUserInfo(address _user) public view returns (UserInfo[] memory) {
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        UserInfo storage userDeposit = userInfo[msg.sender][0];
        
        UserInfo[] memory eachUserInfo = userInfo[msg.sender];
        
        for(uint256 i = 0; i < eachUserInfo.length; i++) {
            if(eachUserInfo[i].pId == _pid) {
                userDeposit = userInfo[msg.sender][i];
            }
        }

        uint256 rewradAmount = myDeposit.rewardAmountHistory;
        ...
        if(canHarvest(_pid, address(msg.sender)) == true) {
            ...
            userDeposit.rewardAmountHistory = rewradAmount.add(myDeposit.rewardAmount);
            myDeposit.lastUpdatedTime = block.timestamp;
            userDeposit.lastUpdatedTime = block.timestamp;
        }
    ...
    function getAPR () public view returns (uint256) {
        uint256 APR = REWARD_PER_DAY.mul(365).mul(100).div(totalLPToken);
        return APR;
    }
    
    function setPerRewardAmount (uint256 _perRewardAmount) external onlyOwner {
        REWARD_PER_DAY = _perRewardAmount;
    }

    // Update reward variables of the given pool to be up-to-date.
    function updatePool(uint256 _pid, address _user) public  returns (uint256, uint256, uint256)  {
        DepositInfo storage myDeposit = depositInfo[_user][_pid][0];
        UserInfo storage userDeposit = userInfo[_user][0];
        ...
        uint256 rewardPerMin = REWARD_PER_DAY.div(1440);
        uint256 lpSupply = myDeposit.amount;
        ...
        if((block.timestamp.sub(myDeposit.startDeposit)).div(60 * 60 *24) < 30) {
            ...
            myDeposit.taxFee = 0;
            userDeposit.taxFee = 0;
        }        
        uint256 tempAmount2;     
        if(myDeposit.taxFee != 0) {
            tempAmount2 = rewardAmount - rewardAmount.mul(myDeposit.taxFee).div(100); // this is working fine. because amount = amount - amount * tax fee
            rewardAmount = tempAmount2;
        }
        else 

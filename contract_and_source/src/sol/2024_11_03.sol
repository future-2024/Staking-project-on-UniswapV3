...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public constant DEFAULT_HARVEST_INTERVAL = 60; // 1 mins;            // User can harvest in next 1 minutes as default
    uint256 public constant MAX_HARVEST_INTERVAL = 120;    // 2 mins               // User cannot harvest or withdraw in next 1 day from deposited time.

    uint256 public REWARD_PER_DAY = 10000000000000000000000000;           // 10K haha per day
    ...
    event BugFix(uint256 indexed token1, uint256 indexed token2);
    ...
    function getPerUserInfo() public view returns (uint256, UserInfo[] memory) {
        uint256 eachUserAmount;
        UserInfo[] memory eachUserInfo = userInfo[msg.sender];
        for(uint256 i = 0; i < eachUserInfo.length; i++) {
            eachUserAmount = eachUserAmount + eachUserInfo[i].amountPerNFT;
        }
        return (eachUserAmount, eachUserInfo);
    }
    ...
    function getAPR () public view returns (uint256) {
        uint256 APR = REWARD_PER_DAY.div(totalLPToken).mul(100);
        return APR;
    }
    ...
    function updatePool(uint256 _pid, address _user) public  returns (uint256, uint256, uint256)  {
        ...
        uint256 rewardPerMin = REWARD_PER_DAY / 60 / 24;
        uint256 lpSupply = myDeposit.amount;
        uint256 rewardRate = (block.timestamp.sub(myDeposit.lastUpdatedTime)).div(60); //  now time - last deposit time / 60 right?
        uint256 rewardAmount = lpSupply.div(totalLPToken).mul(rewardRate).mul(rewardPerMin);  

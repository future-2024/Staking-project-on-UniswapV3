...
import abiNftManager from '@/abi/nftManager.json'
/*
...
if (testnet) {
  ...
  ADDR_DIAMOND_MASTERCHEF = '0x0F0db8340a8D66fdE275B75FCFE659B4Af148a77'
  ...
  ADDR_NFTMANAGER = '0xb39d21718e24DE36fDD8c5C73e928B5EFF33d6C2'
  ...
  ADDR_DIAMOND_MASTERCHEF = '0x0F0db8340a8D66fdE275B75FCFE659B4Af148a77'
  ADDR_LP_FTM_DIAMOND = '0xb39d21718e24DE36fDD8c5C73e928B5EFF33d6C2'
  ADDR_LP_TOKEN_2 = ''
...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        init(state) {
            ...
            state.contracts.nftManager = new window.web3.eth.Contract(abiNftManager, ADDR_NFTMANAGER);
...
//               state.Jackpot.status = 0          //
        ...
        read_diamond(state) {
          ...
          state.contracts.tokenDiamond.methods.currentDay().call().then((ret)=>{
            if (ret > 0)
            {
              ...
              .call().then((ret)=>{
                state.diamond.totalLPTokens = ret;
              })
        ...
        read_masterchef(state) {
            if(state.account){
              state.contracts.tokenMasterchef.methods.getPerUserInfo().call({
                from: state.account.address
              }).then(ret=>{
                state.farming.perUserInfo = ret;
              }).catch((error)=>{
                console.error("tokenDiamondExchanger.userClaimedDiamond",error)
              })
              state.contracts.nftManager.methods.balanceOf(state.account.address).call().then(ret => {
                for(let i = 0; i < ret; i++) {
                  state.contracts.nftManager.methods.tokenOfOwnerByIndex(state.account.address, i).call().then(ret1=> {
                    if(state.nftItem.length < ret) {
                      state.nftItem.push(ret1);
                    }
                  })
                }
              })
            }
            // Total staked from contract all over users
            state.contracts.tokenMasterchef.methods.totalLPToken().call().then(ret=>{
              state.farming.totalLPtokenAmount = BigNumber(ret);
            }).catch((error)=>{
              console.error("tokenDiamondExchanger.userClaimedDiamond",error)
            })

            // Total Earned from contract all over users
            state.contracts.tokenMasterchef.methods.totalHahaRewards().call().then(ret=>{
              state.farming.totalLPtokenEarned = BigNumber(ret);
            }).catch((error)=>{
              console.error("tokenDiamondExchanger.userClaimedDiamond",error)
            })
            state.contracts.tokenMasterchef.methods.getAPR().call().then(ret=>{
              console.log('apr', ret);
              state.farming.APRValue = ret;
            }).catch((error)=>{
              console.error("tokenDiamondExchanger.userClaimedDiamond",error)
            })
          ...
          // state.contracts.totalLPToken[0].methods.getReserves()
          // .call().then((ret)=>{
          //   state.liquidity.amountDiamond = BigNumber(ret[1]);
          //   state.liquidity.amountFTM = BigNumber(ret[0]);
          // })
    ...
    actions: {
      getDepositInfo({state, commit}, params) {
        state.contracts.tokenMasterchef.methods.depositInfo(params.user, params.pId, 0)
        .call().then((ret)=>{
          state.farming.depositInfo.push(ret);         
        })
      },
...
//            commit('read_jackpot');
      ...
      approvePairMaster({state}, params) {
        console.log('ok---');
        console.log(ADDR_DIAMOND_MASTERCHEF)
...
//      commit('read_diamondExchanger')
      ...
      harvest({state,commit},params) {
        return new Promise((resolve)=>{
          state.contracts.tokenMasterchef.methods.harvest(params.amountHarvest)
            .send({
              from: state.account.address
            }).then((ret)=>{
              commit("read_masterchef");

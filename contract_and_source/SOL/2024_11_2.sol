...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    uint256 public constant DEFAULT_HARVEST_INTERVAL = 60; // minutes;            // User can harvest in next 1 minutes as default
    uint256 public constant MAX_HARVEST_INTERVAL = 120;                   // User cannot harvest or withdraw in next 1 day from deposited time.
    ...
    function harvest(uint256 _pid) public nonReentrant {
        ...
        myDeposit.rewardAmountHistory = rewradAmount.add(myDeposit.rewardAmount); 
        ...
        if(canHarvest(_pid, address(msg.sender)) == true)
            ...
            totalHahaRewards = totalHahaRewards.add(myDeposit.rewardAmount);                                                  // User total reward amount from contract will be increased.
    ...
    function updatePool(uint256 _pid, address _user) public  returns (uint256, uint256, uint256, uint256, uint256, uint256)  {

        DepositInfo storage myDeposit =  depositInfo[_user][_pid][0];

        uint256 lpSupply = myDeposit.amount;
        uint256 rewardRate = (block.timestamp.sub(myDeposit.lastUpdatedTime)).div(60);
        uint256 rewardAmount;
        uint256 tempAmount1 = lpSupply.div(totalLPToken).mul(rewardRate);

        rewardAmount = tempAmount1;
        myDeposit.taxFee = 30 - (block.timestamp.sub(myDeposit.startDeposit)).div(60 * 60 *24);
        uint256 tempAmount2;
        if(myDeposit.taxFee != 0) {
            tempAmount2 = rewardAmount.mul(myDeposit.taxFee).div(100);
            rewardAmount = tempAmount2;
        }
        else 
            rewardAmount = rewardAmount;
        myDeposit.rewardAmount = rewardAmount;
        return (rewardAmount, rewardAmount, tempAmount1, tempAmount1, myDeposit.taxFee, lpSupply);
    }
}